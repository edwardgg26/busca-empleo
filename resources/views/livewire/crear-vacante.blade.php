<form class="grid grid-cols-1 md:grid-cols-2 gap-4" wire:submit='save' novalidate>
    <div class="md:col-span-2 md:w-1/2 md:pe-2">
    ...
    <div>
        <x-inputs.input-label for="imagen" :value="__('Imagen')" />
        <x-inputs.file class="block mt-2 w-full" 
                       wire:model="imagen"
                       id="imagen"/>
        <x-activity-indicator wire:loading wire:target="imagen"/>
        @if ($imagen)
            @if ($imagen->extension() === "jpg" || $imagen->extension() === "jpeg" || $imagen->extension() === "png" )
                <img class="mt-2 max-w-80 w-full" src="{{$imagen->temporaryUrl()}}" alt="Imagen de vacante">
            @else
                <x-inputs.input-error :messages="['El archivo debe ser jpg, jpeg o png.']" class="mt-2" />
            @endif
        @endif
        <x-inputs.input-error :messages="$errors->get('imagen')" class="mt-2" />
    </div>

    <div class="flex items-center md:col-span-2 ">
        <x-inputs.primary-button wire:loading.attr='disabled' wire:target="save">
            <p wire:loading.remove wire:target="save">Guardar</p>
            <x-activity-indicator class="text-white dark:text-gray-900 w-3 h-3" wire:loading wire:target="save"/>
